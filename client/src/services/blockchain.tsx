...
export const getProvider = (
  ...
  return new Program<TwitterPlatform>(idl as any, provider)
}
...
export const likePost = async (
    ...
    const [likePda] = PublicKey.findProgramAddressSync(
        [Buffer.from('like'), publicKey.toBuffer(), new BN(postId).toArrayLike(Buffer, 'le', 8)],
        program.programId
    )
...
export const fetchAllUserProfiles = async (program: Program<TwitterPlatform>) => {
    try {
...
export const fetchPostComments = async (
    ...
    try {
        ...
        return allComments.filter(comment => comment.account.postId.toNumber() === postId)
    } catch (error) {
...
export const fetchUserFollows = async (
    ...
    try {
        ...
        return allFollows.filter(follow => follow.account.follower.equals(userPublicKey))
    } catch (error) {
...
export const fetchUserFollowers = async (
    ...
    try {
        const allFollows = await program.account.follow.all()
        return allFollows.filter(follow => follow.account.following.equals(userPublicKey))
    } catch (error) {
...
export const checkIfLiked = async (
    ...
    const [likePda] = PublicKey.findProgramAddressSync(
        [Buffer.from('like'), userPublicKey.toBuffer(), new BN(postId).toArrayLike(Buffer, 'le', 8)],
        program.programId
    )
...
export const checkIfFollowing = async (
    ...
    const [followPda] = PublicKey.findProgramAddressSync(
        [Buffer.from('follow'), followerPublicKey.toBuffer(), followingPublicKey.toBuffer()],
        program.programId
    )
